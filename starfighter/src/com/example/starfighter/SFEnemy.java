...
public class SFEnemy {
	...
	/**
	 * Konstruktor
	 * @param typ lodi, smer od kud prileti
	 * */
	public SFEnemy(int type, int direction){
		enemyType = type;
		attackDirection = direction;
		//startovni y pozice
		posY = (randomPos.nextFloat()*4)+4; //pohyb na ose od 0 po 4 a pricteme 4 aby byl ven z obr.
	...
	public float getNextScoutX(){
		if(attackDirection == SFEngine.ATTACK_LEFT){
			return (float)((SFEngine.BEZIER_X_4*(Math.pow(posT,3)))+
					...
					(SFEngine.BEZIER_X_2*3*posT*(Math.pow((1-posT), 2)))+
					(SFEngine.BEZIER_X_1*(Math.pow((1-posT), 3))));
		}
		else{
		...
		return (float)((SFEngine.BEZIER_X_1*(Math.pow(posT,3)))+
				...
				(SFEngine.BEZIER_X_3*3*posT*(Math.pow((1-posT), 2)))+
				(SFEngine.BEZIER_X_4*(Math.pow((1-posT), 3))));
		}
	}
	...
	public float getNextScoutY(){
		return (float)((SFEngine.BEZIER_Y_1*(Math.pow(posT,3)))+
				...
				(SFEngine.BEZIER_Y_3*3*posT*(Math.pow((1-posT), 2)))+
				(SFEngine.BEZIER_Y_4*(Math.pow((1-posT), 3))));
	...
	public void draw(GL10 gl, int[]spriteSheet){
		gl.glBindTexture(GL10.GL_TEXTURE_2D, spriteSheet[0]); //svaze texturu s cilem-nahraje a je pripravena
